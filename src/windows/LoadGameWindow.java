...
public class LoadGameWindow extends JPanel {
    ...
    LoadGameWindow()
    {
        ...
        // now, attempt to start up that game
        try
        {
            Savegame chosen = Savegame.read(selected);
            GameManager.init(Optional.of(chosen), chosen.getSessionID(), chosen.getGameVariant(), NetworkUtils.getLocalIPAddPort());
        }
        catch (Exception e)
        {
            Logger.getGlobal().severe("There was a problem loading the savegame.");
            e.printStackTrace();
        }
