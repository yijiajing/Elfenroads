        ...
        // Player intends to place a magic spell
            ...
            if (counter.getType() == MagicSpellType.EXCHANGE) {
                if (road.getRegionType() == RegionType.RIVER || road.getRegionType() == RegionType.LAKE
                    ...
                    inExchange = true;
                    GameScreen.displayMessage("You just started an Exchange! Please click on another road with which you would like to exchange the transportation counter on the road you just selected. You must make certain that the exchanged transportation counters are legal on the roads they are exchanged to.");
                }
